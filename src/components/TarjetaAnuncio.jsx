...
function TarjetaAnuncio({ titulo, precio, descripcion }) {
  return (
    <div className="card">
      <h3>{titulo}</h3>
      <p style={{ color: '#666', marginBottom: '8px' }}>{descripcion}</p>
      <p style={{ fontSize: '1.2rem', fontWeight: 'bold', color: '#16a34a' }}>${precio}</p>
      <button style={{ marginTop: '8px', padding: '6px 16px', borderRadius: '6px', border: '1px solid #ccc', cursor: 'pointer' }}>
        Contactar
      </button>
    </div>
  )
